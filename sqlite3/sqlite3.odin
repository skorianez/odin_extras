...
callback_proc :: #type proc "c" (data: rawptr, a: c.int, b: [^]cstring, c: [^]cstring) -> ResultCode

/*
call back example
callback :: proc "c" (data: rawptr, argc: c.int, argv: [^]cstring, azColName: [^]cstring) -> sql.ResultCode { 
    context = runtime.default_context()
    for i in 0..<argc {
        fmt.printf("%s = %s\n", azColName[i], argv[i])
    }
	return sql.ResultCode.OK
}
*/
...
foreign sqlite {
	...
	exec :: proc(db: ^sqlite3, sql: cstring, call: callback_proc, arg: rawptr, errmsg: ^[^]c.char) -> ResultCode ---;
